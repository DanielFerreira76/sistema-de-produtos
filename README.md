# 🛒 Consulta de Produtos

Projeto desenvolvido com HTML, CSS, JavaScript e JSON que simula um sistema de consulta de produtos. O usuário informa um código e a aplicação exibe dinamicamente informações como nome, imagem e preço do produto.

> **Em desenvolvimento:** futuramente o projeto contará com suporte à leitura de código de barras físico.

## ✨ Funcionalidades

- Consulta de produtos por código
- Busca dinâmica utilizando JSON
- Exibição de imagem do produto
- Exibição de nome, código e preço
- Formatação de preço em Real (R$)

## 🛠 Tecnologias

- HTML5
- CSS3
- JavaScript
- JSON

## 👨‍💻 Autor

Desenvolvido por **Daniel F. Vieira**.
